...
const ICON_OPTIONS = [
  'calculate', 'psychology', 'menu_book', 'translate', 'functions', 
  'bar_chart', 'change_history', 'category', 'school', 'edit_note',
  'language', 'public', 'flag', 'description', 'find_in_page',
  'notes', 'segment', 'data_object', 'numbers', 'chat', 'star', 'history_edu'
];
...
const ManageLatihan = () => {
  ...
  const fetchQuestions = async (subjectId, topicId, page = 1) => {
    ...
    try {
      const res = await soalService.list({ subject_id: subjectId, topic_id: topicId, page, limit: 20 });
      // Backend returns { success: true, data: [...] } directly
      const questionsList = Array.isArray(res.data?.data) ? res.data.data : [];
      setQuestions(questionsList);
      setQuestionsPagination({ total: questionsList.length, page, totalPages: 1 });
    } catch (err) {
  ...
  const handleDeleteQuestion = async (id) => {
    ...
    try {
      ...
      fetchQuestions(selectedSubject.id, selectedTopic?.id, questionsPagination.page);
    } catch (err) {
  ...
  const handleDeleteSubject = async (e, id) => {
    e.stopPropagation();
    if (!window.confirm('Hapus subtes ini? Semua data terkait akan ikut terhapus.')) return;
    try {
  ...
  const PLAN_OPTIONS = [
    ...
    { value: 'sultan', label: 'Sultan', color: 'bg-yellow-100 text-yellow-800' },
  ];
      ...
      <div className="max-w-[1440px] mx-auto px-6 lg:px-10">
        ...
        <div className="pt-8 mb-4">
          <nav className="flex items-center gap-2 text-[12px] font-semibold text-[#727687] uppercase tracking-wider">
            ...
            <span className={selectedSubject ? 'text-[#727687]' : 'text-[#0050cb]'}>Subtes</span>
            {selectedSubject && (
              <>
                <span className="material-symbols-outlined text-[14px]">chevron_right</span>
                <button onClick={() => { setSelectedTopic(null); }} className={`hover:text-[#0050cb] transition-colors ${selectedTopic ? 'text-[#727687]' : 'text-[#0050cb]'}`}>{selectedSubject.title}</button>
              </>
            )}
            {selectedTopic && (
              <>
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <div>
            <h1 className="text-[48px] font-bold text-[#191b24] leading-tight">
              {selectedTopic ? `Kelola Soal: ${selectedTopic.title}` : selectedSubject ? `Kelola Topik: ${selectedSubject.title}` : 'Kelola Kategori Latihan'}
            </h1>
            <p className="text-[18px] text-[#424656] mt-2 max-w-2xl">
              {selectedTopic
                ? `Daftar soal untuk topik ${selectedTopic.title} (${selectedSubject.title}). Total: ${questionsPagination.total} soal.`
                : selectedSubject 
          ...
          <div className="flex items-center gap-4">
            {(selectedSubject || selectedTopic) && (
              <button 
                onClick={() => selectedTopic ? setSelectedTopic(null) : setSelectedSubject(null)}
            ...
            {selectedTopic ? (
              ...
              <button 
                onClick={() => selectedSubject ? handleOpenTopicModal() : handleOpenSubjectModal()}
                className="bg-[#0050cb] text-white px-6 py-3.5 rounded-xl font-bold flex items-center gap-2 hover:shadow-lg hover:shadow-[#0050cb]/20 transition-all active:translate-y-px"
              >
        ...
        {selectedTopic ? (
          ...
          <div className="space-y-6 animate-fade-in-up">
            ...
            {showImportSection && (
              <div className="bg-white border border-[#c2c6d8]/30 rounded-2xl p-8 shadow-sm animate-fade-in">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <h3 className="text-[20px] font-bold text-[#191b24]">Import Soal dari Excel</h3>
                    <p className="text-[14px] text-[#424656] mt-1">Soal akan diimport ke subtes <strong>{selectedSubject.title}</strong></p>
                  </div>
                ...
                <div className="flex flex-col md:flex-row gap-6">
                  ...
                  >
                    ...
                    {importFile ? (
                      <div>
                        <span className="material-symbols-outlined text-green-600 text-[32px] mb-2">check_circle</span>
                        <p className="font-bold text-[#191b24]">{importFile.name}</p>
                        <p className="text-[13px] text-[#727687] mt-1"><strong>{importPreview.length}</strong> soal terdeteksi</p>
                        <button type="button" onClick={(e) => { e.stopPropagation(); setImportFile(null); setImportPreview([]); }} className="text-[#ba1a1a] text-[13px] font-bold mt-2 hover:underline">Hapus</button>
                      </div>
                    ) : (
                      <div>
                        <span className="material-symbols-outlined text-[#727687] text-[32px] mb-2">upload_file</span>
                        <p className="font-bold text-[#424656]">Klik untuk pilih file Excel</p>
                        <p className="text-[12px] text-[#727687] mt-1">.xlsx atau .xls</p>
                      </div>
                    )}
                  </div>
                  {/* Config */}
                  <div className="w-full md:w-64 space-y-4">
                    ...
                    <button
                      ...
                      className="w-full bg-[#0050cb] text-white py-3 rounded-xl font-bold flex items-center justify-center gap-2 hover:shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {importLoading ? (
                        <><span className="material-symbols-outlined animate-spin text-[18px]">progress_activity</span> Mengimport...</>
                      ) : (
                        <><span className="material-symbols-outlined text-[18px]">cloud_upload</span> Import {importPreview.length > 0 ? `${importPreview.length} Soal` : ''}</>
                      )}
                    </button>
                  </div>
                </div>
            ...
            {questionsLoading ? (
              ...
              <div className="text-center py-20 bg-white rounded-[40px] border border-[#c2c6d8]/30">
                ...
                <p className="text-[#727687] font-medium">Belum ada soal untuk subtes ini.</p>
                <p className="text-[14px] text-[#727687] mt-2 mb-6">Import soal dari file Excel atau gunakan Generate AI.</p>
                {!showImportSection && (
                  <button 
                    ...
                    <div key={q.id} className="bg-white border border-[#c2c6d8]/30 rounded-2xl overflow-hidden hover:shadow-md transition-all">
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-[15px] text-[#191b24] font-medium line-clamp-2">{q.content}</p>
                              ...
                              <div key={c.id} className={`p-3 rounded-xl border text-[14px] flex items-start gap-3 ${
                                ...
                                <div>
                                  ...
                                  {c.is_correct && c.explanation && (
                                    <p className="text-[12px] text-green-700 mt-1 italic">{c.explanation}</p>
                                  )}
                                </div>
                              </div>
                            ))}
                ...
                {questionsPagination.totalPages > 1 && (
                  <div className="flex items-center justify-center gap-2 pt-4">
                    {Array.from({ length: questionsPagination.totalPages }, (_, i) => i + 1).slice(0, 10).map(p => (
                      <button
                        key={p}
                        onClick={() => fetchQuestions(selectedSubject.id, selectedTopic.id, p)}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {loading ? (
              [1,2,3,4,5,6].map(i => (
                <div key={i} className="bg-white h-[350px] rounded-3xl border border-[#c2c6d8]/30 animate-pulse"></div>
              ))
            ) : (
              (subjects || []).map((s) => (
                <div 
                ...
                >
                  <div className="flex justify-between items-start mb-8">
                    ...
                    >
                      <span className="material-symbols-outlined text-[28px]" style={{ fontVariationSettings: "'FILL' 1" }}>{s.icon || 'school'}</span>
                    </div>
                    <div className="flex gap-1">
                      <button 
                        onClick={(e) => { e.stopPropagation(); handleOpenSubjectModal(s); }}
                        className="p-2 text-[#727687] hover:text-[#0050cb] hover:bg-[#dae1ff]/30 rounded-full transition-colors"
                      >
                        <span className="material-symbols-outlined text-[20px]">edit</span>
                      </button>
                  ...
                  <p className="text-[14px] text-[#424656] mb-4 line-clamp-2">
                    {s.description || 'No description available for this module.'}
                  </p>
                  <span className={`inline-flex items-center gap-1 text-[11px] font-bold uppercase tracking-wider px-3 py-1 rounded-lg ${
                    s.required_plan === 'sultan' ? 'bg-yellow-100 text-yellow-800' :
                    s.required_plan === 'premium' ? 'bg-blue-100 text-blue-700' :
                    'bg-gray-100 text-gray-600'
                  }`}>
                    <span className="material-symbols-outlined text-[14px]">
                      {s.required_plan === 'sultan' ? 'star' : s.required_plan === 'premium' ? 'diamond' : 'lock_open'}
                    </span>
                    {s.required_plan === 'sultan' ? 'Sultan' : s.required_plan === 'premium' ? 'Premium' : 'Gratis'}
                  </span>
          ...
          <div className="space-y-6 animate-fade-in-up">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {(topics || []).map((t) => (
                <div key={t.id} className="bg-white border border-[#c2c6d8]/30 rounded-[32px] p-6 hover:shadow-xl transition-all group border-l-4 border-l-[#0050cb]">
                  <div className="flex justify-between items-start mb-4">
                    <div className="w-12 h-12 rounded-2xl bg-[#dae1ff]/50 flex items-center justify-center text-[#0050cb]">
                      <span className="material-symbols-outlined text-[24px]">{t.icon || 'notes'}</span>
                    </div>
                    <div className="flex gap-1">
                      <button 
                  ...
                  <h4 className="text-[18px] font-bold text-[#191b24] mb-1">{t.title}</h4>
                  <p className="text-[13px] text-[#424656] mb-4 line-clamp-2">{t.description}</p>
              ...
              >
                ...
                <span className="font-bold text-[#424656] group-hover:text-[#0050cb]">Tambah Topik Baru</span>
              </button>
            </div>
            {(!topics || topics.length === 0) && (
              <div className="text-center py-20 bg-white rounded-[40px] border border-[#c2c6d8]/30">
                <span className="material-symbols-outlined text-[64px] text-[#c2c6d8] mb-4">folder_open</span>
                <p className="text-[#727687] font-medium">Belum ada topik untuk subtes ini. Mulai buat sekarang!</p>
              </div>
            )}
          </div>
        )}
          ...
          <section className="mt-20 bg-[#0066ff] text-white rounded-[32px] p-10 lg:p-12 relative overflow-hidden shadow-2xl">
            ...
            <div className="relative z-10 flex flex-col lg:flex-row justify-between items-center gap-12">
              <div className="max-w-xl">
                <h2 className="text-[32px] lg:text-[40px] font-bold mb-4 leading-tight">Practice Performance Overview</h2>
                <p className="text-white/80 text-[18px] leading-relaxed">Real-time distribution metrics across all topics. Overall content quality score is currently at <span className="text-[#00c1fd] font-bold underline">94.2%</span> based on student feedback.</p>
              </div>
              <div className="flex gap-16 text-center">
                <div>
                  <p className="text-[48px] lg:text-[64px] font-bold leading-none mb-2">6,555</p>
                  <p className="text-[12px] font-extrabold uppercase tracking-[0.2em] opacity-70">Questions Total</p>
                </div>
                <div>
                  <p className="text-[48px] lg:text-[64px] font-bold leading-none mb-2">24%</p>
                  <p className="text-[12px] font-extrabold uppercase tracking-[0.2em] opacity-70">Hard Difficulty</p>
                </div>
              </div>
            </div>
          </section>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-[40px] w-full max-w-lg overflow-hidden shadow-2xl animate-scale-in">
            <div className="p-10 border-b border-[#c2c6d8]/20 flex justify-between items-center bg-[#faf8ff]">
              <div>
                <h3 className="font-bold text-[28px] text-[#191b24]">{editingItem ? 'Edit Subtes' : 'Tambah Subtes Baru'}</h3>
                <p className="text-[14px] text-[#727687]">Konfigurasi detail kategori latihan di bawah ini.</p>
              </div>
              <button onClick={() => setShowSubjectModal(false)} className="w-12 h-12 flex items-center justify-center rounded-full hover:bg-white shadow-sm transition-all text-[#424656]">
                <span className="material-symbols-outlined">close</span>
              </button>
            </div>
            <form onSubmit={handleSubjectSubmit} className="p-10 space-y-8 overflow-y-auto max-h-[70vh] custom-scrollbar">
              <div className="space-y-6">
                <div>
                  <label className="block text-[14px] font-bold text-[#191b24] mb-2">Nama Subtes</label>
                  <input 
                    required
                ...
                <div>
                  <label className="block text-[14px] font-bold text-[#191b24] mb-2">Deskripsi</label>
                  <textarea 
                    rows={3}
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div>
                    ...
                    <div className="flex items-center gap-3 bg-[#f2f3ff] rounded-2xl px-4 py-1 border border-[#c2c6d8]/20">
                      <input type="color" className="w-8 h-8 rounded-lg cursor-pointer" value={subjectForm.iconColor} onChange={e => setSubjectForm({...subjectForm, iconColor: e.target.value})} />
                      <input 
                        className="w-full bg-transparent border-none py-3 outline-none text-[14px] font-mono"
                        value={subjectForm.iconColor}
                        onChange={e => setSubjectForm({...subjectForm, iconColor: e.target.value})}
                      />
                    </div>
                  </div>
                  <div>
                    <label className="block text-[14px] font-bold text-[#191b24] mb-2">Warna BG (Hex)</label>
                    <div className="flex items-center gap-3 bg-[#f2f3ff] rounded-2xl px-4 py-1 border border-[#c2c6d8]/20">
                      <input type="color" className="w-8 h-8 rounded-lg cursor-pointer" value={subjectForm.bgColor} onChange={e => setSubjectForm({...subjectForm, bgColor: e.target.value})} />
                      <input 
                        className="w-full bg-transparent border-none py-3 outline-none text-[14px] font-mono"
                        value={subjectForm.bgColor}
                        onChange={e => setSubjectForm({...subjectForm, bgColor: e.target.value})}
                      />
                    </div>
                  </div>
                </div>
                <div>
                  <label className="block text-[14px] font-bold text-[#191b24] mb-2">Paket Minimum</label>
                  <div className="flex gap-3">
                    {PLAN_OPTIONS.map(opt => (
                      <button
                        key={opt.value}
                        type="button"
                        onClick={() => setSubjectForm({...subjectForm, requiredPlan: opt.value})}
                  ...
                  <p className="text-[12px] text-[#727687] mt-2">Hanya user dengan paket ini atau di atasnya yang bisa mengakses.</p>
                </div>
                <div>
                  <label className="block text-[14px] font-bold text-[#191b24] mb-4">Pilih Ikon Visual</label>
              ...
              <div className="pt-4">
                ...
                >
                  {isSubmitting ? 'Menyimpan...' : (editingItem ? 'Perbarui Subtes' : 'Simpan Subtes')}
                </button>
              </div>
            </form>
          </div>
      ...
      {showTopicModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-[40px] w-full max-w-2xl overflow-hidden shadow-2xl animate-scale-in">
            <div className="p-10 border-b border-[#c2c6d8]/20 flex justify-between items-center bg-[#faf8ff]">
              <div>
                <h3 className="font-bold text-[28px] text-[#191b24]">{editingItem ? 'Edit Topik' : 'Tambah Topik Baru'}</h3>
                <p className="text-[14px] text-[#727687]">Konfigurasi detail topik latihan untuk {selectedSubject?.title}.</p>
              </div>
              <button onClick={() => setShowTopicModal(false)} className="w-12 h-12 flex items-center justify-center rounded-full hover:bg-white shadow-sm transition-all text-[#424656]">
                <span className="material-symbols-outlined">close</span>
              </button>
            </div>
            <form onSubmit={handleTopicSubmit} className="p-10 grid grid-cols-1 md:grid-cols-2 gap-10 overflow-y-auto max-h-[80vh] custom-scrollbar">
              <div className="space-y-6">
                <div>
                  <label className="block text-[14px] font-bold text-[#191b24] mb-2">Nama Topik</label>
                  <input 
                    required
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[14px] font-bold text-[#191b24] mb-2">Jumlah Soal</label>
                    <input 
                      type="number"
                  ...
                  <div>
                    <label className="block text-[14px] font-bold text-[#191b24] mb-2">Level</label>
                    <select 
              ...
              <div className="space-y-6 flex flex-col">
                <div>
                  <label className="block text-[14px] font-bold text-[#191b24] mb-2">Tipe Kartu</label>
                  <select 
                    className="w-full bg-[#f2f3ff] border border-[#c2c6d8]/20 rounded-2xl px-6 py-4 outline-none focus:ring-2 focus:ring-[#0050cb] appearance-none"
                ...
                <div className="flex flex-col gap-4 p-4 bg-[#f2f3ff] rounded-2xl border border-[#c2c6d8]/10">
                  <label className="flex items-center gap-3 cursor-pointer group">
                    <input type="checkbox" checked={topicForm.isPopular} onChange={e => setTopicForm({...topicForm, isPopular: e.target.checked})} className="w-5 h-5 rounded border-[#c2c6d8] text-[#0050cb] focus:ring-[#0050cb]" />
                    <span className="text-[14px] font-bold text-[#424656] group-hover:text-[#0050cb] transition-colors">Beri Tag "Populer"</span>
                  </label>
                  <label className="flex items-center gap-3 cursor-pointer group">
                    <input type="checkbox" checked={topicForm.isFeatured} onChange={e => setTopicForm({...topicForm, isFeatured: e.target.checked})} className="w-5 h-5 rounded border-[#c2c6d8] text-[#0050cb] focus:ring-[#0050cb]" />
                    <span className="text-[14px] font-bold text-[#424656] group-hover:text-[#0050cb] transition-colors">Beri Tag "Spesial"</span>
                  </label>
                </div>
                <div>
                  <label className="block text-[14px] font-bold text-[#191b24] mb-3">Pilih Ikon</label>
                ...
                <div className="mt-auto">
                  ...
                  >
                    {isSubmitting ? 'Menyimpan...' : (editingItem ? 'Perbarui Topik' : 'Simpan Topik')}
                  </button>
                </div>
              </div>
            </form>
